...
const AttributesModal = ({open, data, onClose, onSaved}) => {
    ...
    useEffect(() => {
        const allSelected =
            allowable.length > 0 && exists.length === allowable.length;
    ...
    const getSelectedValue = useCallback(
        key_id =>
            exists.find(e => e.key_id === key_id)?.attr_value_ids?.[0]?.id ?? null,
        [exists]
    );
