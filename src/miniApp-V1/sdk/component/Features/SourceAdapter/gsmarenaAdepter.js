import {monthsMap, runExtractors, sanitizeInputNumber} from "../utils.js";
...
export default function gsmarenaAdapter(features_array) {
    const releaseDate = () => {
    ...
    const memory = () => {
        ...
        for (const item of features_array) {
            if (!memorySpecs) {
                memorySpecs = getMemory(item);
            }
            if (memorySpecs) {
                break;
            }
        }
        return memorySpecs ? {memorySpecs} : null
    }
    ...
    const cpu = () => {
        ...
        return cpuSpecs ? {cpuSpecs} : null
    };


    const watchCpu = () => {
        const getWatchCpu = (item) => {
            const cpuWatchSpecs = item['Platform']?.['Chipset'] || null;
            const cpuName = item['Platform']?.['CPU'] || null;
            return {cpuWatchSpecs, cpuName};
        };
        for (const item of features_array) {
            const cpuData = getWatchCpu(item);
            if (cpuData.cpuWatchSpecs || cpuData.cpuName) {
                return cpuData;
            }
        }
        return null;
    ...
    const sensors = () => {
        const getSensors = (item) => {
            return sensorsSpecs =
                item['Features']?.['Sensors'] || null
        }

        let sensorsSpecs;
        for (const item of features_array) {
            if (!sensorsSpecs) {
                sensorsSpecs = getSensors(item);
            }
            if (sensorsSpecs) {
                break;
            }
        }
        return sensorsSpecs ? {sensorsSpecs} : null
    }
